...
class Organization(Resource):
    ...
    def get(cls):
        ...
        if not is_wrong_token:
            try:
                ...
                return OrganizationDAO.get_organization(
                    int(user["id"]), representative_name
                )
    ...
    @organizations_ns.expect(
        auth_header_parser, update_organization_request_model, validate=True
    )
    def put(cls):
        """
        ...
        if not is_wrong_token:
            ...
            is_field_valid = expected_fields_validator(
                data, update_organization_request_model
            )
...
class OrganizationsList(Resource):
    ...
    def get(cls):
        ...
        per_page = request.args.get(
            "per_page", default=DEFAULT_ORGANIZATIONS_PER_PAGE, type=int
        )

        is_wrong_token = validate_token(token)

        if not is_wrong_token:
            return OrganizationDAO.list_organizations(
                request.args.get("name", ""), page, per_page, token
            )
        return is_wrong_token
...
class Program(Resource):
    ...
    def get(cls, organization_id, program_id):
        ...
        if not is_wrong_token:
            return ProgramDAO.get_program_by_program_id(
                organization_id, program_id, token
            )
        return is_wrong_token
