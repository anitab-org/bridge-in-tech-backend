...
class MyProfilePersonalDetails(Resource):
    ...
    def put(cls):
        ...
        is_field_valid = expected_fields_validator(
            data, update_user_details_request_body_model
        )
        if not is_field_valid.get("is_field_valid"):
            return is_field_valid.get("message"), HTTPStatus.BAD_REQUEST
...
class MyProfileAdditionalInfo(Resource):
    ...
    @users_ns.response(HTTPStatus.BAD_REQUEST.value, "Invalid input.")
    @users_ns.response(
        HTTPStatus.NOT_FOUND.value, f"{messages.ADDITIONAL_INFORMATION_DOES_NOT_EXIST}"
    )
    @users_ns.expect(auth_header_parser, validate=True)
    def get(cls):
        """
        ...
        if not is_wrong_token:
            ...
            if not result:
                return (
                    messages.ADDITIONAL_INFORMATION_DOES_NOT_EXIST,
    ...
    @users_ns.expect(
        auth_header_parser, user_extension_request_body_model, validate=True
    )
    def put(cls):
        """
        ...
        if not is_wrong_token:
            ...
            is_field_valid = expected_fields_validator(
                data, user_extension_request_body_model
            )
            if not is_field_valid.get("is_field_valid"):
                return is_field_valid.get("message"), HTTPStatus.BAD_REQUEST
...
class MyProfilePersonalBackground(Resource):
    ...
    def get(cls):
        ...
        if not is_wrong_token:
            user_json = AUTH_COOKIE["user"].value
            user = ast.literal_eval(user_json)
            result = PersonalBackgroundDAO.get_user_personal_background_info(
                int(user["id"])
            )
            if not result:
    ...
    @users_ns.expect(
        auth_header_parser, user_personal_background_request_body_model, validate=True
    )
    def put(cls):
        """
        ...
        if not is_wrong_token:
            ...
            is_field_valid = expected_fields_validator(
                data, user_personal_background_request_body_model
            )
            if not is_field_valid.get("is_field_valid"):
                return is_field_valid.get("message"), HTTPStatus.BAD_REQUEST
...
class UsersList(Resource):
    ...
    def get(cls):
        ...
        per_page = request.args.get(
            "per_page", default=DEFAULT_USERS_PER_PAGE, type=int
        )
