...
class OrganizationDAO:
            ...
            if representative_additional_info.is_organization_rep:
                ...
                if not organization:
                    organization = OrganizationModel(
                        int(user["id"]), name, email, address, website, timezone
                    )
                    return update(
                        organization,
                        data,
                        messages.ORGANIZATION_SUCCESSFULLY_CREATED,
                        HTTPStatus.CREATED,
                    )
                ...
                return update(
                    organization,
                    data,
                    messages.ORGANIZATION_SUCCESSFULLY_UPDATED,
                    HTTPStatus.OK,
                )
    ...
    def list_organizations(name, page, per_page, token):
        ...
        organizations_list = (
            OrganizationModel.query.filter(
                func.lower(OrganizationModel.name).contains(name.lower())
            )
        ...
        list_of_organizations = [
            organization.json() for organization in organizations_list
        ]
        user_params = {
            "search": "",
            "page": DEFAULT_PAGE,
        ...
        users = http_response_checker(
            get_request("/users/verified", token, params=user_params)
        )
            ...
            for organization in list_of_organizations:
                if organization["status"].value == "Publish":
                    ...
                    if organization["rep_id"] == int(user["id"]):
                        logged_in_user_organization = get_named_tuple_result(
                            get_organization(int(user["id"]), user["name"])
                        )
                        if logged_in_user_organization.status_code == 200:
                            organizations_list.append(
                                logged_in_user_organization.message
                            )
                    user_additional_info = UserExtensionModel.find_by_user_id(
                        int(user["id"])
                    )
                    for user in users.message:
                        member_additional_info = UserExtensionModel.find_by_user_id(
                            user["id"]
                        )
                        try:
                            if member_additional_info.is_organization_rep:
                                if organization["rep_id"] == user["id"]:
                                    ...
                                    try:
                                        readable_join_date = (
                                            convert_timestamp_to_human_date(
                                                ...
                                                user_additional_info.timezone.value,
                                            )
                                        )
                                    except AttributeError:
                                        readable_join_date = (
                                    ...
                                    organization_item = {
                                        ...
                                        "representative_department": organization[
                                            "rep_department"
                                        ],
    ...
    try:
        if representative_additional_info.is_organization_rep:
            ...
            try:
                ...
                readable_join_date = convert_timestamp_to_human_date(
                    result.join_date, representative_additional_info.timezone.value
                )
                return {
                    "id": result.id,
