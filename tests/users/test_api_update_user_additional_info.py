...
class TestUpdateUserAdditionalInfoApi(BaseTestCase):
    ...
    def test_api_dao_update_user_additional_info_successfully(self):
        ...
        additional_info = {"phone": "123-456-456", "mobile": "", "personal_website": ""}

        user_extension = UserExtensionModel(
            user_id=self.test_user_data.id, timezone="EUROPE_KIEV"
        )
        ...
        with self.client:
            response = self.client.put(
                ...
                data=json.dumps(dict(self.correct_payload_update_additional_info)),
                follow_redirects=True,
                content_type="application/json",
            )
        ...
        self.assertEqual(
            ...
            self.correct_payload_update_additional_info["is_organization_rep"],
        )
        self.assertEqual(
            test_user_additional_info_data.timezone.value,
            self.correct_payload_update_additional_info["timezone"],
        )
        self.assertEqual(
            test_user_additional_info_data.additional_info["phone"],
            self.correct_payload_update_additional_info["phone"],
        )
        self.assertEqual(
            test_user_additional_info_data.additional_info["mobile"],
            self.correct_payload_update_additional_info["mobile"],
        )
        self.assertEqual(
            test_user_additional_info_data.additional_info["personal_website"],
            self.correct_payload_update_additional_info["personal_website"],
        )
    ...
    def test_api_dao_update_additional_info_with_invalid_timezone(self):
        ...
        with self.client:
            response = self.client.put(
                "/user/additional_info",
                headers={"Authorization": AUTH_COOKIE["Authorization"].value},
                data=json.dumps(dict(invalid_timezone_update_additional_info)),
                follow_redirects=True,
                content_type="application/json",
            )
